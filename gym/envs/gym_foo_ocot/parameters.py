...
class Parameters:
    def __init__(self, randstate):
        ...
        self.episode_max_length = 1000 #0  # enforcing an artificial terminal # the max small time steps that 1 episode has # consistent with steps in gym _init_.py

        # self.num_epochs = 10000         # number of training epochs
        self.simu_len = self.episode_max_length #  10             # length of the busy cycle that repeats itself # 1 episode = 1 simulation, it has simu_len time steps/actions. 
        self.num_ex = 1                # number of sequences # number of jobs that come at the same time
        ...
        self.max_job_len =  10 # 6 # 15          # maximum duration of new jobs
        self.res_slot = 10             # maximum number of available resource slots
        self.max_job_size =  5 # 3 # 5 # 10         # maximum resource request of new work

        self.backlog_size = 500  # 100         # backlog queue size
        ...
        self.new_job_rate = 0.5  # 0.7        # lambda in new job arrival Poisson Process
        ...
        self.dist = Dist(self.num_res, self.max_job_size, self.max_job_len, randstate)
        

        # graphical representation
        assert self.backlog_size % self.time_horizon == 0  # such that it can be converted into an image
        self.backlog_width = int(math.ceil(self.backlog_size / float(self.time_horizon))) # backlog_size = 60, 20 time_horizon rows * 3 columns
