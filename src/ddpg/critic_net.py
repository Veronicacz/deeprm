...
class CriticNet:
    ...
    def __init__(self, num_states, num_actions):
        ...
        with self.g.as_default():
            ...
            # # # critic_q_model parameters:
            # self.W1_c, self.B1_c, self.W4_c,self.B4_c,self.W2_c, self.W2_action_c, self.B2_c, self.W3_c, self.B3_c,\
            #     self.critic_q_model, self.critic_state_in, self.critic_action_in = self.create_critic_net(num_states, num_actions)

            # # create target_q_model:
            # self.t_W1_c, self.t_B1_c, self.t_W4_c,self.t_B4_c,self.t_W2_c, self.t_W2_action_c, self.t_B2_c, self.t_W3_c, self.t_B3_c,\
            #     self.t_critic_q_model, self.t_critic_state_in, self.t_critic_action_in = self.create_critic_net(num_states, num_actions)
            ...
            file = "/home/veronica/Desktop/model/critic_net_model_zz0.ckpt"
            ...
            #     self.t_B4_c.assign(TAU * self.B4_c + (1 - TAU) * self.t_B4_c),
            #     self.t_W2_c.assign(TAU * self.W2_c + (1 - TAU) * self.t_W2_c),
            #     self.t_W2_action_c.assign(TAU * self.W2_action_c + (1 - TAU) * self.t_W2_action_c),
            #     self.t_B2_c.assign(TAU * self.B2_c + (1 - TAU) * self.t_B2_c),
            #     self.t_W3_c.assign(TAU * self.W3_c + (1 - TAU) * self.t_W3_c),
    ...
    def create_critic_net(self, num_states=2520, num_actions=6):  # 4 & 1
        N_HIDDEN_1 = 400 # 1500 # 2500 # 400
        N_HIDDEN_2 = 300 # 1000 # 2000 # 300
        N_HIDDEN_3 = 600
        ...
        W3_c = tf.Variable(tf.random_uniform([N_HIDDEN_2, 1], -0.01, 0.01)) # -0.003, 0.003))
        B3_c = tf.Variable(tf.random_uniform([1], -0.01, 0.01)) #-0.003, 0.003))
    ...
    #     B1_c = tf.Variable(tf.random_uniform(
    #         [N_HIDDEN_1], -1 / math.sqrt(num_states), 1 / math.sqrt(num_states)))
    #     W4_c = tf.Variable(tf.random_uniform(
    #         [N_HIDDEN_1, N_HIDDEN_2], -1 / math.sqrt(N_HIDDEN_1), 1 / math.sqrt(N_HIDDEN_1)))
    #     B4_c = tf.Variable(tf.random_uniform(
    #         [N_HIDDEN_2], -1 / math.sqrt(N_HIDDEN_1), 1 / math.sqrt(N_HIDDEN_1)))
    #     W2_c = tf.Variable(tf.random_uniform(
    #         [N_HIDDEN_2, N_HIDDEN_3], -1 / math.sqrt(N_HIDDEN_2 + num_actions), 1 / math.sqrt(N_HIDDEN_2 + num_actions)))
    #     W2_action_c = tf.Variable(tf.random_uniform(
    #         [num_actions, N_HIDDEN_3], -1 / math.sqrt(N_HIDDEN_2 + num_actions), 1 / math.sqrt(N_HIDDEN_2 + num_actions)))
    #     B2_c = tf.Variable(tf.random_uniform(
    #         [N_HIDDEN_3], -1 / math.sqrt(N_HIDDEN_2 + num_actions), 1 / math.sqrt(N_HIDDEN_2 + num_actions)))
    #     W3_c = tf.Variable(tf.random_uniform([N_HIDDEN_3, 1], -0.01, 0.01)) # -0.003, 0.003))
    #     B3_c = tf.Variable(tf.random_uniform([1], -0.01, 0.01)) #-0.003, 0.003))

    #     H0_c = tf.nn.relu(tf.matmul(critic_state_in, W1_c) + B1_c)
    #     H1_c = tf.nn.relu(tf.matmul(H0_c, W4_c) + B4_c)  #softplus
    #     H2_c = tf.nn.relu(tf.matmul(H1_c, W2_c) + tf.matmul(critic_action_in, W2_action_c) + B2_c)  # tanh

    #     critic_q_model = tf.matmul(H2_c, W3_c) + B3_c #tf.nn.softmax(tf.matmul(H2_c, W3_c) + B3_c) # tf.matmul(H2_c, W3_c) + B3_c

    #     return W1_c, B1_c, W4_c,B4_c,W2_c, W2_action_c, B2_c, W3_c, B3_c, critic_q_model, critic_state_in, critic_action_in
    ...
    def save_model(self, ep):
        file = "/home/veronica/Desktop/model/critic_net_model_zz" + str(ep) + ".ckpt"
