...
class RsEnv:
	...
	def __init__(self, nw_len_seqs=None, nw_size_seqs=None,
		...
		self.pa = env.parameters.Parameters(self.ransta)
   ...
   #          	self.read_work_files()
	...
	def get_sjf_len_action(self):
		...
		length = np.zeros(self.pa.num_nw) #np.zeros(int(self.pa.num_nw/2))

		for i in range(self.pa.num_nw): #range(int(self.pa.num_nw/2)):
			tem_length = 0
		...
		if np.any(length[:] > 0):
			...
			for i in range(self.pa.num_nw): #(int(self.pa.num_nw/2)):
				if length[i] != 0:
					tem.append(length[i])
			...
			true_index = np.where(length == tem_value)[0][0]
 ...
 #        # plt.pause(0.01)  # automatic
	...
	def step(self, a, repeat=False):
		...
		if status == 'MoveOn':
			...
			if self.end == "no_new_job":  # end of new job sequence
				...
				if not done:
					...
					if self.seq_idx < self.pa.simu_len:  # otherwise, end of new job sequence, i.e. no new jobs
						...
						new_job1 = self.get_new_job_from_seq(self.seq_no+1, self.seq_idx)

						if np.any(new_job1.res_vec > 0):  # a new job comes

							print("new job comes")
							print(new_job1.order)
							...
							for i in range(self.pa.num_nw):
								if self.job_slot.slot[i] is None:  # put in new visible job slots
									self.job_slot.slot[i] = new_job1
									self.job_record.record[new_job1.id] = new_job1
									to_backlog = False
									break
							if to_backlog:
								if self.job_backlog.curr_size < self.pa.backlog_size:
									self.job_backlog.backlog[self.job_backlog.curr_size] = new_job1
									self.job_backlog.curr_size += 1
									self.job_record.record[new_job.id] = new_job1
								else:  # abort, backlog full
									print("Backlog is full.")
									self.disslog.log[self.disslog.size] = new_job1
									self.disslog.size += 1
                                	# exit(1)
							self.extra_info.new_job_comes()
			...
			f=open("/home/veronica/Desktop/env/job_profile/0726/sjf/act_job_info.txt", "a")
			f.write("%s\n" % tem_ac)
			f.close()			
			# exit()
...
# 		f=open("/home/veronica/Desktop/env/job_profile/0726/sjf/job_queue_num.txt", "a")
...
# 		f=open("/home/veronica/Desktop/env/job_profile/0726/sjf/job_info.txt", "a")
# #		tem_str1 = "nw_len," + self.nw_len_seqs + "\n" 
# 		# for item in self.nw_len_seqs:
# 		f.write("%s\n" % tem_l)
# 		f.close()
  ...
  #                                          [self.pa.num_ex, self.pa.simu_len, self.pa.num_clu])
		...
		f1=open("/home/veronica/Desktop/env/job_profile/0726/sjf/nwlen.txt", "a")
#		tem_str1 = "nw_len," + self.nw_len_seqs + "\n" 
		for item in self.nw_len_seqs:
			f1.write("%s\n" % item)
		f1.close()
...
#		f1.close()
		...
		f2=open("/home/veronica/Desktop/env/job_profile/0726/sjf/nwsize.txt", "a")
...
#		f2.write(tem_str2)
		...
		f=open("/home/veronica/Desktop/env/job_profile/0726/sjf/nworder.txt", "a")
#		tem_str2 = "nw_size, " + self.nw_size_seqs + "\n"
...
#		f2.write(tem_str2)
		...
		f=open("/home/veronica/Desktop/env/job_profile/0726/sjf/nwtrans.txt", "a")
#		tem_str2 = "nw_size, " + self.nw_size_seqs + "\n"
...
#		f2.write(tem_str2)
		...
		f=open("/home/veronica/Desktop/env/job_profile/0726/sjf/clu_trans.txt", "a")
#		tem_str2 = "nw_size, " + self.nw_size_seqs + "\n"
...
#		f2.write(tem_str2)
		...
		for seqt in range(self.pa.simu_len):
			for i in range(self.pa.num_clu):
				...
				tem_load = tem_load + 1 #1.5 * tem_load
		...
		f3=open("/home/veronica/Desktop/env/job_profile/0726/sjf/workload.txt", "a")
#		tem_str2 = "nw_size, " + self.nw_size_seqs + "\n"
		f3.write("%s\n" % workload)
#		f2.write(tem_str2)
		f3.close()		
