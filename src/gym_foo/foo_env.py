...
from gym.envs.gym_foo.parameters import *
		...
		for i in range(self.pa.num_res):
			...
			for j in range(self.pa.num_nw):
				if self.job_slot.slot[j] is not None:  # fill in a block of work   # self.job_slot.slot = [None] * pa.num_nw
					...
					occup_color = np.unique(self.machine.canvas[:])
					# WARNING: there should be enough colors in the color map
					for color in self.machine.colormap:
						if color not in occup_color:
							n_color = color
							break
					image_repr[: self.job_slot.slot[j].len, ir_pt: ir_pt + self.job_slot.slot[j].res_vec[i]] = n_color
...
#		self.plot_state()
	...
	def reset(self):
		...
		f=open("/home/veronica/Desktop/env/job_profile/0706/freezetime/normal/disslog.txt", "a")
#		tem_str1 = "nw_len," + self.nw_len_seqs + "\n" 
		...
		f1=open("/home/veronica/Desktop/env/job_profile/0706/freezetime/normal/nwlen.txt", "a")
#		tem_str1 = "nw_len," + self.nw_len_seqs + "\n" 
		for item in self.nw_len_seqs:
			f1.write("%s\n" % item)
		f1.close()
...
#		f1.close()
		...
		f2=open("/home/veronica/Desktop/env/job_profile/0706/freezetime/normal/nwsize.txt", "a")
...
class Machine:
    def __init__(self, pa, ransta):
        ...
        self.colormap = np.arange(1, 200, 5)  # pixel value, from 1 to 255, it cannot be 0, as canvas's initial value is 0
